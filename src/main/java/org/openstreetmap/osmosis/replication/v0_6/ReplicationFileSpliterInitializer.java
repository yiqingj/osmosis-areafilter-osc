 ...
 */
public class ReplicationFileSpliterInitializer implements RunnableTask {
	
	private static final String LOCK_FILE_NAME = "download.lock";
	private static final String CONFIG_FILE_NAME = "configuration.txt";
	private static final String CONFIG_REGION_FILE_NAME = "area-list.json";
	private static final String CONFIG_RESOURCE = "impl/replicationFileMergerConfiguration.txt";
	private static final String CONFIG_RESOURCE_REGION = "impl/area-list.json";
	 ...
	 */
	public ReplicationFileSpliterInitializer(File workingDirectory) {
	...
	private void initializeDirectory() {
		...
		File regionFile = new File(workingDirectory, CONFIG_REGION_FILE_NAME);
		if (regionFile.exists()) {
			throw new OsmosisRuntimeException("Region Config file " + regionFile + " already exists.");
		}
		resourceFileManager.copyResourceToFile(getClass(), CONFIG_RESOURCE_REGION, regionFile);
		
		ReplicationFileRegionConfiguration config = new ReplicationFileRegionConfiguration(regionFile);
		for (Region region : config.getRegionList()) {
			// Create the data directory.
			dataDirectory = new File(workingDirectory, region.getKey());
